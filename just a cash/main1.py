...
class hippo(pygame.sprite.Sprite): #inharited from sprite parents (make this class be a part of pygame)
    ...
    def update(self): #overdrive update
        ...
        #15 frame per one im 
            ...
            if type(hand_tracking.index_finger_position_y) is float:
                self.rect.y = hand_tracking.index_finger_position_y
                self.rect.x = hand_tracking.index_finger_position_x
                print(self.rect,hand_tracking.index_finger_position_x)

            if eye_tracking.left_eye_state or eye_tracking.right_eye_state:
                if eye_tracking.left_eye_state == 'closed' or eye_tracking.right_eye_state == 'closed' and self.mount == False:
                    self.mount = True
                    self.velocity = -7
                if eye_tracking.left_eye_state == 'open' or eye_tracking.right_eye_state == 'open' and self.mount == True:
                    self.mount = False
...
#create hippo
hippo_group = pygame.sprite.Group()   # like a list but store sprite and  will use method for individual
flappy = hippo(70,screen_heigth//2)
...
eye_tracking = EyesTracking()
mediapipe_thread = MediapipeThread(eye_tracking)
...
while run == True:
    ...
    #score part
    text_convernt(score,font,(255,255,255),screen_width-50,screen_heigth-50)
