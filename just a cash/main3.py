...
def main(charactor):
    if charactor ==1:
        mout_tracking = MountTracking()
        media = MediapipeThread(mout_tracking)
        media.start()
    game_frame_rate = 30
    scrolling_ground = 0
    scrolling_speed = 4 
        ...
        def update(self): #overdrive update
            ...
            #15 frame per one im 
                ...
                if charactor ==1:
                    ...
                    if mout_tracking.mount_state == 'open':
                        self.velocity = -7
    ...
    class charactor01(pygame.sprite.Sprite): #inharited from sprite parents (make this class be a part of pygame)
        ...
        def update(self): #overdrive update
            ...
            #15 frame per one im 
                ...
                #flape
                if mout_tracking.mount_state == 'open' and self.mount == False:
                    self.velocity = -7
                    self.mount = True
                if mout_tracking.mount_state == 'closed' and self.mount == True:
                    self.mount == False
