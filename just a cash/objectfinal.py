...
class Gen_button_from_image(pygame.sprite.Sprite):
    def __init__(self,image_part,x,y):
        ...
        self.rect = self.image.get_rect()
        self.rect.center = (x,y)
        self.clicked_sound = self.click_sound1 = pygame.mixer.Sound('sound\zipclick.flac')
        self.click_state = False

    def update(self):
        cersor_pos = pygame.mouse.get_pos() #index got x and y coordinate
        '''
        if self.rect.collidepoint(cersor_pos) and pygame.mouse.get_pressed()[0] == True\
            and self.click_state == False:
            self.click_state = True
        if self.rect.collidepoint(cersor_pos) and pygame.mouse.get_pressed()[0] == False\
