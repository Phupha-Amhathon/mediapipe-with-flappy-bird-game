...
def choosing_charactor():
    ...
    charactor_number3_button = gen_button_from_text('mosquitoe',400,300,pygame.font.SysFont('arial',20),(0,0,0))
    ...
    button_group.add(charactor_number1_button,charactor_number2_button,
                     ...
                     back_button)
    charactor_id = {0:hippo,1:snake,2:mosquitoe}
    #gameplay()
    button_group.draw(screen)
    print('thaiaifdlkjfaljdfkljalskdf',len(charactor_id))
    while run == True:
        for i in  range(len(button_group)):
            button_group.update()
            
            if button_group.sprites()[i].click == 1:
               
                if i < len(charactor_id):
                    gameplay(charactor_id[i])
                    sys.exit()
                else:
                    menu()
                    sys.exit()
...
def gameplay(charactor_name):
    ...
    charactor = charactor_name(200,screen_heigth//2,False,False)
