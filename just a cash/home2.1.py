...
from object import FloatingObstacle,ZigzagObstacle
...
class LevelManager:
    def __init__(self,score):
        ...
        self.next_level_score = 5  # Score needed to reach the next level
        self.movement_types = ['horizon_rock','vertical_rock','sinusoidal','oscillating','zigzag','teleport','accelerate']
    def adjust_difficulty(self):
        # Adjusting speed and frequency based on the level
        if self.level == 1:
            obstacle_speed = 3
            spawn_delay = 2000  # Delay between obstacle spawns in ms
            movement_type = self.movement_types[6]  # Simple sinusoidal movement
            x_position = 860
            y_positon = random.randint(50,600)
        elif self.level == 2:
            obstacle_speed = 4
            spawn_delay = 1700
            movement_type = self.movement_types[1]
            x_position = random.randint(150,800)
            y_positon = 660
        elif self.level == 3:
            obstacle_speed = 5
            spawn_delay = 1500
            movement_type = rd.choice(self.movement_types[0:2])
            x_position = random.randint(150,800)
            y_positon = random.randint(50,600)
        elif self.level == 4:
            obstacle_speed = 6
            spawn_delay = 1300
            movement_type = self.movement_types[3]
            x_position = random.randint(150,800)
            y_positon = 660
        elif self.level == 5:
            obstacle_speed = 6
            spawn_delay = 1300
            movement_type = self.movement_types[1:4]
            x_position = 860
            y_positon = random.randint(50,600)
        elif self.level == 6:
            obstacle_speed = 7
            spawn_delay = 1000
            movement_type = 'random'  # Can be mixed (add more complex patterns here)
        else:
            return None
        return obstacle_speed, spawn_delay, movement_type,x_position,y_positon

    def spawn_obstacles(self):
        if self.adjust_difficulty():
            obstacle_speed, spawn_delay, movement_type,x_position,y_position = self.adjust_difficulty()
            current_time = pygame.time.get_ticks()
            if current_time - self.spawn_time > spawn_delay:
                self.spawn_time = current_time
                obstacle = FloatingObstacle(x_position, y_position, obstacle_speed, movement_type)
    ...
    def check_level_progress(self):
        ...
        if self.score >= self.next_level_score:
            ...
            if self.level > 5:
                self.level = 5  # Cap at Level 5 for now
                print("You've reached the final level!")
